...
import { missionClientGate } from "@/lib/session-presence";
...
export default function MissionClient({
  ...
  useEffect(() => {
    async function load() {
      ...
      const gate = sessionConfirmed
        ? "ready"
        : missionClientGate({ clientUser: user, cookieHeader });

      if (!user) {
        // Cookie-present or server-confirmed: stay on missions. Do not paint
        // Sign in → /login under a signed-in header.
        if (gate === "signed-out") {
          setSignedOut(true);
        }
        setLoading(false);
